package com.digit.customcamera.internal;
...
import com.digit.customcamera.ICallback;
import com.digit.customcamera.R;
import com.digit.customcamera.util.CameraUtil;
import com.digit.customcamera.util.Degrees;
import com.digit.customcamera.util.ImageUtil;
import com.digit.customcamera.util.ManufacturerUtil;
...
import static com.digit.customcamera.internal.BaseCaptureActivity.CAMERA_POSITION_BACK;
import static com.digit.customcamera.internal.BaseCaptureActivity.CAMERA_POSITION_FRONT;
import static com.digit.customcamera.internal.BaseCaptureActivity.CAMERA_POSITION_UNKNOWN;
import static com.digit.customcamera.internal.BaseCaptureActivity.FLASH_MODE_ALWAYS_ON;
import static com.digit.customcamera.internal.BaseCaptureActivity.FLASH_MODE_AUTO;
import static com.digit.customcamera.internal.BaseCaptureActivity.FLASH_MODE_OFF;
...
public class CameraFragment extends BaseCameraFragment implements View.OnClickListener {
    ...
    private static Camera.Size choosePictureSize(BaseCaptureInterface ci, List<Camera.Size> choices) {
        List<Camera.Size> backupSize = new ArrayList<>();
        for (Camera.Size size : choices) {
            if (size.width == size.height * ci.videoPreferredAspect())
                backupSize.add(size);
        }
        if (backupSize.size() > 0) {
            Camera.Size size = null;
            int pos = backupSize.size() / 2;
            if (pos >= 1) {
                size = backupSize.get(pos-1);
            } else {
                size = Collections.min(backupSize, new CompareSizesByArea());
            }
            Log.d("CameraFragment", "Using resolution: " + size.width + "x" + size.height);
            return size;
        } else {
            LOG(CameraFragment.class, "Couldn't find any suitable size");
...
//                    choosePictureSize(mInterface, parameters.getSupportedPictureSizes());
